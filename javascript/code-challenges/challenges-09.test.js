...
const maxInArray = (arr) => {
  ...
  return arr.reduce ((e,i)=>Math.max(e,i)); 
};
...
const checkValues = (obj, value) => {
  ...
  return obj[Object.keys(obj)] === value ? true : false
};
...
const hasChildrenValues = (arr, character) => {
  ...
  let newarr=arr.filter(item =>{
    ...
    return item.name === character && item.hasOwnProperty('children')
  })

  let truthly = newarr.length === 0 ? false : true;
  return truthly
};
